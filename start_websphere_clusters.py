...
for cluster in clusters:
    ...
    for server in servers:
        ...
        server_id = AdminConfig.showAttribute(server, "serverId")
        print("Starting server: " + server_name + " on node: " + node_name)
        process = Runtime.getRuntime().exec("wsadmin.sh -lang jython -c 'AdminControl.startServer(\"" + server_name + "\", \"" + node_name + "\", \"" + server_id + "\")'")
        process.waitFor()
